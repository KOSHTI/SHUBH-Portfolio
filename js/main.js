...
document.addEventListener('DOMContentLoaded', () => {
    // Function to show the floating image panel
    const showFloatingImagePanel = (imageUrl) => {
        const floatingImagePanel = document.getElementById('floatingImagePanel');
        const floatingImage = document.getElementById('floatingImage');
        
        if (imageUrl) {
            floatingImage.src = imageUrl; // Set the image URL
            floatingImagePanel.style.display = 'block'; // Show the panel
            document.body.style.overflow = 'hidden'; // Disable body scrolling while panel is open
        }
    };

    // Function to hide the floating image panel
    const hideFloatingImagePanel = () => {
        const floatingImagePanel = document.getElementById('floatingImagePanel');
        floatingImagePanel.style.display = 'none'; // Hide the panel
        document.body.style.overflow = ''; // Re-enable body scrolling
    };

    // Add event listener to the close button
    const closeBtn = document.querySelector('.floating-image-panel .close-btn');
    if (closeBtn) {
        closeBtn.addEventListener('click', hideFloatingImagePanel);
    }
    ...
    imageLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            if (imageUrl && imageUrl !== '#') {
                showFloatingImagePanel(imageUrl);
            } else {
                console.warn('Image link href is missing or invalid:', link);
            }
        });
    });

    // Optionally close the image panel if clicking outside the panel
    document.addEventListener('click', (e) => {
        const floatingImagePanel = document.getElementById('floatingImagePanel');
        if (floatingImagePanel && !floatingImagePanel.contains(e.target)) {
            hideFloatingImagePanel();
        }
    });

    // Close the floating image panel with Escape key
